...
async def create_scan(
    ...
    inspector_id: Annotated[str | None, Form()] = None,
    ...
    try:
        scan, _report, _fields = scanning.run_scan(
            session, payload,
            inspector_id=inspector_id, source=source, package_type=package_type,
...
@router.post("/scans/{scan_id}/status", response_model=ScanDetail)
def set_status(
    scan_id: str,
...
@router.post("/scans/{scan_id}/notice", status_code=201)
def create_notice(
    scan_id: str,
    body: NoticeRequest,
    session: Annotated[Session, Depends(get_session)],
...
@router.post("/notices/{notice_id}/sign")
def sign_notice(
    notice_id: str,
    body: SignRequest,
    session: Annotated[Session, Depends(get_session)],
    ...
    notice.signed_by = body.officer_id
